...
class FlyingToasters2014 : public SimpleRobot
{
	...
	Joystick Operator; //Logitech Joystick Attack 3
	Talon Arm; //Raise and lower BAG motor
	Talon Launcher; //Two CIM motors for launching the ball
...
public:
	...
	
	{
		...
		Threshold threshold(114, 136, 101, 255, 214, 255);	//HSV threshold criteria, ranges are in that order ie. Hue is 60-100
		...
		while (IsAutonomous() && IsEnabled()) {
			if (AutoMode.Get() != true){
				...
				if (PistonCounter < 1){
					Arm.Set(0.0);
					Launcher.Set(1.0);
					ds->PrintfLine(DriverStationLCD::kMain_Line6, "TWO BALLLLLLLLLL!!!!!!!!!");
	...
	void OperatorControl(void)
	{
		...
		float angle;
		float I = 0.1;
		float PIDError; //Launching High Goal From Line
		float PIDError2; //Launching High Goal From Low Goal
		float PIDError3;
		float SetPoint = 4.0;
		float SetPoint2 = 4.0;
		float SetPoint3 = 2.48;
		float IntegralError;
		float ErrorRefresh;
		float P = 0.6;
		while (IsOperatorControl())
		{
			...
			PIDError = (SetPoint - angle);
			PIDError2 = (SetPoint2 - angle);
			PIDError3 = (SetPoint3 - angle);
			ds->PrintfLine(DriverStationLCD::kUser_Line3, "Pot: %f", angle);
			ds->PrintfLine(DriverStationLCD::kUser_Line4, "PIDError: %f", PIDError);
			ds->PrintfLine(DriverStationLCD::kUser_Line5, "Int Error %f", IntegralError);
			ds->UpdateLCD();
			Auton.Reset();
			...
			if (Operator.GetRawButton(4) != true && Operator.GetRawButton(5) != true)
			{
				IntegralError = 0;
			}
			...
			{
				ErrorRefresh = 0.005*PIDError;
				IntegralError = IntegralError + ErrorRefresh;
				Arm.Set(-1*((PIDError * P) + IntegralError * I));
				Launcher.Set(1.0);
			}	
			if (Operator.GetRawButton(2) == true)
			{
				Arm.Set(-1*(PIDError3 * P));
				Launcher.Set(-0.475);
			}
			if (Operator.GetRawButton(5) == true)
			{
				ErrorRefresh = 0.005*PIDError2;
				IntegralError = IntegralError + ErrorRefresh;
				Arm.Set(-1*((PIDError2 * P) + IntegralError * I));
				Launcher.Set(0.70);
			}
			...
			if (range <= 220 && range >= 216)
			{
				ds->PrintfLine(DriverStationLCD::kUser_Line2, "Fire");
				LED.Set(Relay::kOn);
			}
			else if (range > 220 && range < 216) 
			{
				LED.Set(Relay::kOff);
			}
